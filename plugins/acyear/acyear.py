            ...
            if article.triportour == 'trip':
              #The boundary is august/september
                if int(article.date.strftime('%m')) > 8:
                    ...
                    yearlist.append(row(article.date, year1 + '-' + year2, article))
                else:
                    year1 = str(int(article.date.strftime('%Y')) - 1)
                    year2 = article.date.strftime('%Y')
                    yearlist.append(row(article.date, year1 + '-' + year2, article))

    #Sort the list by date (the full date, not academic year) and make available to
    #generator context (so the templates can access it)
    generator.context['yearlist'] = sorted(yearlist, key=lambda x: x.date, reverse=True)
    print "acyear: Success!"
